...
async function runBacktest(symbol, signalType, confidence) {
  ...
  try {
    ...
    // === استراتيجية Backtest واقعية — شروط مرنة ===
    // نختبر كل 5 شمعات لتوليد عينة كافية
    const step = Math.max(1, Math.floor(closes.length / 200)); // حد أقصى 200 إشارة
    for (let i = 20; i < closes.length - 10; i += step) {
      ...
      const ema10 = calculateEMA(window, Math.min(10, window.length));

      // شروط مرنة — تُولّد إشارات كافية للتحليل الإحصائي
      const longSignal = signalType === 'long' && (
        (closes[i] > ema20 && rsi < 70) ||
        (rsi < 35) // oversold
      );
      const shortSignal = signalType === 'short' && (
        (closes[i] < ema20 && rsi > 30) ||
        (rsi > 65) // overbought
      );
