...
async function getKlines(symbol, timeframe) {
  ...
  // محاولة Binance أولاً للإطارات المعتادة
  const binanceIntervalMap = { '1h': '1h', '4h': '4h', '1d': '1d' };
  const binanceLimitMap = { '1h': 48, '4h': 84, '1d': 90 };
  const binanceInterval = binanceIntervalMap[timeframe];
  if (binanceInterval) {
    const binanceData = await getBinanceKlines(symbol, binanceInterval, binanceLimitMap[timeframe] || 90);
    if (binanceData.length >= 5) {
      const ttl = timeframe === '1h' ? 900 : timeframe === '4h' ? 1800 : 7200;
      await setCache(cacheKey, binanceData, ttl);
      return binanceData;
    }
  }
...
// مجاني، بدون حدود، 3 سنوات كاملة، أسرع بكثير من CoinGecko
async function getBinanceKlines(symbol, interval = '1d', limit = 1000) {
  const base = symbol.replace('/USDT', '').toUpperCase();
  ...
  try {
    ...
    const ttl = interval === '1d' ? 12 * 3600 : 3600;
    await setCache(cacheKey, klines, ttl);
    logger.info(`🐆 Binance OK: ${pair} ${klines.length} شمعة (${interval})`);
    return klines;
  } catch (e) {
    logger.debug(`🐆 Binance فشل ${pair}: ${e.response?.status || e.message}`);
    return [];
  }
}

// ==================== HISTORICAL DATA (Binance أولاً + CoinGecko fallback) ====================
async function getHistoricalDaily(symbol, days = 365) {
  const cacheKey = `hist_${symbol}_${days}d`;
  const cached = await getCached(cacheKey);
  if (cached) return cached;

  // محاولة 1: Binance (الأسرع والأكثر موثوقية)
  const limit = Math.min(days, 1000);
  const binanceData = await getBinanceKlines(symbol, '1d', limit);
  if (binanceData.length >= 10) {
    await setCache(cacheKey, binanceData, 12 * 3600);
    return binanceData;
  }

  // محاولة 2: CoinGecko /ohlc (fallback)
  const base = symbol.replace('/USDT', '').toUpperCase();
  const geckoId = GECKO_IDS[base] || base.toLowerCase();
  try {
    const resp = await geckoRequest(
