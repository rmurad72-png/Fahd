...
const geckoQueue = { queue: [], processing: false, lastCall: 0, minInterval: 1200 };
  ...
  while (geckoQueue.queue.length > 0) {
    ...
    try {
      ...
      if (e.response?.status === 429) {
        logger.warn(`🐆 CoinGecko 429 — انتظار 15 ثانية`);
        await sleep(15000);
  ...
  try {
    const response = await axios.get(
      'https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest',
      {
        ...
        params: { limit: 150, convert: 'USD', sort: 'market_cap', sort_dir: 'desc', market_type: 'spot' },
        timeout: 12000
      }
    );
...
async function getHistoricalDaily(symbol, days = 1095) {
  ...
  const cached = await getCached(cacheKey);
  if (cached) { logger.info(`🐆 Hist CACHE: ${symbol}`); return cached; }

  const base = symbol.replace('/USDT', '').toUpperCase();
  const geckoId = GECKO_IDS[base] || base.toLowerCase();
  const hasKey = !!process.env.COINGECKO_API_KEY;

  logger.info(`🐆 Hist FETCH: ${symbol} days=${days} geckoId=${geckoId} key=${hasKey}`);

  // محاولة 1: /market_chart (أفضل للـ Backtest)
  try {
    const params = { vs_currency: 'usd', days, interval: 'daily' };
    if (hasKey) params.x_cg_demo_api_key = process.env.COINGECKO_API_KEY;
    const resp = await geckoRequest(
      `https://api.coingecko.com/api/v3/coins/${geckoId}/market_chart`,
      params, 25000
    );
    const prices = resp.data?.prices || [];
    logger.info(`🐆 Hist market_chart: ${symbol} prices=${prices.length} status=${resp.status}`);
    if (prices.length >= 30) {
      ...
      const klines = prices.map((p, i) => {
        ...
        return {
          time: p[0], open: prevClose,
          high: Math.max(close, prevClose) * 1.01,
          low: Math.min(close, prevClose) * 0.99,
          close, volume: volumes[i]?.[1] || 0
        };
      });
      await setCache(cacheKey, klines, 24 * 3600);
      logger.info(`🐆 Hist OK: ${symbol} ${klines.length} شمعة`);
      return klines;
    }
  } catch (e) {
    logger.warn(`🐆 Hist market_chart FAIL: ${symbol} status=${e.response?.status} ${e.message}`);
  }

  // محاولة 2: /ohlc fallback
  logger.info(`🐆 Hist fallback /ohlc: ${symbol}`);
  try {
    const params2 = { vs_currency: 'usd', days: Math.min(days, 365) };
    if (hasKey) params2.x_cg_demo_api_key = process.env.COINGECKO_API_KEY;
    const resp2 = await geckoRequest(
      `https://api.coingecko.com/api/v3/coins/${geckoId}/ohlc`,
      params2, 20000
    );
    const ohlcData = resp2.data || [];
    logger.info(`🐆 Hist ohlc: ${symbol} candles=${ohlcData.length}`);
    if (ohlcData.length >= 30) {
      const klines = ohlcData.map(k => ({
    ...
    logger.warn(`🐆 Hist ohlc FAIL: ${symbol} ${e.message}`);
  }

  logger.error(`🐆 Hist TOTAL FAIL: ${symbol} — إرجاع مصفوفة فارغة`);
  return [];
}
...
function calculateConfidence(coin, mtf, onChain, type) {
  ...
  else { score += 2; penalties += 5; } // تناقض الاتجاهات = عقوبة

  // MTF Alignment (15) — خُفِّض من 20 لمنع هيمنته
  maxScore += 15;
  const mtfAlignment = mtf?.alignment || 0;
  const mtfBoost = mtf?.confidenceBoost || 0;
  if (mtfAlignment >= 1.0) score += 15;       // 3/3 فريمات
  else if (mtfAlignment >= 0.67) score += 10; // 2/3 فريمات
  else if (mtfAlignment >= 0.33) score += 5;  // 1/3 فريمات
  else { score += 1; penalties += 8; }         // 0/3 = عقوبة كبيرة
  ...
  else { score += 1; penalties += 3; } // حجم منخفض = خطر
  ...
  if (onChain?.btcDominance) {
    ...
    if (dom > 58 && trend === 'bullish' && coin.symbol !== 'BTC') {
      score += 2; penalties += 5;
  ...
  // عملات خارج Top 50 مع MTF=100% مشكوك فيها
  if (rank > 50 && mtfAlignment >= 1.0 && volRatio < 0.05) {
    penalties += 10; // MTF=100% مع حجم ضعيف = إشارة مشبوهة
  }
  ...
  logger.debug(`🐆 Confidence ${coin.symbol}: raw=${rawScore} penalties=${penalties} final=${finalScore} MTF=${(mtfAlignment*100).toFixed(0)}%`);
  return finalScore;
}
