...
async function executeTradesFromScan(userId, scanResult, type, isManual) {
  ...
  for (let i = 0; i < Math.min(slots, eligible.length, type === 'daily' ? 2 : 3); i++) {
    ...
    try {
      ...
      if (zScore > 2.5) {
        logger.warn('🐆 رفض ' + opp.symbol + ': Z-Score ' + zScore + ' > 2.5 تشبع شرائي شديد');
        continue;
      }
      // تحذير عند Z-Score مرتفع
      if (zScore > 1.8) {
        logger.info('🐆 تحذير ' + opp.symbol + ': Z-Score ' + zScore + ' مرتفع — خفض الثقة 8%');
      ...
      // التنفيذ اليدوي: نجلب السعر الحالي للدخول الفوري
      let currentPrice = opp.price;
      if (isManual) {
        try {
          const priceData = await getVerifiedPrice(opp.symbol);
          if (priceData && priceData.price > 0) currentPrice = priceData.price;
        } catch(e) { /* نستخدم سعر المسح */ }
      }
