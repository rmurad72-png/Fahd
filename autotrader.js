...
async function executeTradesFromScan(userId, scanResult, type = 'daily') {
  ...
  for (let i = 0; i < Math.min(slots, eligible.length, type === 'daily' ? 2 : 3); i++) {
    ...
    try {
      ...
      if (fullAnalysis.confidence < (effectiveThreshold - 10)) continue;
      if (fullAnalysis.recommendation === 'avoid') continue;
      // منع تنفيذ صفقة بدون سجل Backtest تاريخي
      if (backtestResult && backtestResult.occurrences === 0) {
        logger.warn('🐆 تخطي ' + opp.symbol + ': Backtest = 0 إشارات — لا سجل تاريخي');
        continue;
      }
