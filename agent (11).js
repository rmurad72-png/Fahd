...
async function quickScanSummary(opportunities, onChain) {
  ...
  const fg = onChain?.fearGreed;
  const fearContext = fg && fg.value < 35
    ? `السوق في منطقة خوف (${fg.value}/100) — الفرص المتاحة اجتازت فلاتر المخاطرة الصارمة، وهذا يعني انتقائية أعلى لا تعارضاً. التاريخ يُظهر أن فرص الخوف أعطت عوائد أفضل على المدى المتوسط.`
    : '';
  const system = `أنت "الفهد 🐆" — تقدم ملخصاً موجزاً لأفضل فرص السوق الفوري. اكتب بالعربية مباشرة دون JSON. إذا كان السوق في منطقة خوف وتوجد فرص، وضّح أن الفرص المعروضة اجتازت فلاتر إضافية وهي انتقائية وليست تناقضاً.`;
...
السعر: $${p > 0 ? (p || 0).toFixed(p < 1 ? 6 : 4) : 'N/A'}
...
MTF: ${mtfData ? (mtfData.dominantTrend || 'محايد') + ' | تناسق ' + Math.round((mtfData.alignment||0)*100) + '%' : 'N/A'}
Z-Score: ${(marketData.zScore || 0).toFixed(2)} — ${marketData.zInterpret || 'N/A'}
...
سعر BTC الحالي: ${btcPriceNow} — استخدم هذا السعر الحقيقي فقط ولا تذكر أسعاراً قديمة.
