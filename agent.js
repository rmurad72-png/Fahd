...
async function analyzeSmartMoney(symbol, marketData) {
  const system = `أنت "الفهد 🐆" — محلل Smart Money متخصص بمستوى صناديق التحوط. اكتب بالعربية مباشرة.`;
  const p = parseFloat(marketData.price) || 0;
  const mtf = marketData.mtf || {};
  const onChain = marketData.onChain || {};
...
async function generateAIForecast(symbol, marketData, backtest) {
  const system = `أنت "الفهد 🐆" — نموذج توقع AI متخصص للأسواق المالية. اكتب بالعربية مباشرة.`;
  const p = parseFloat(marketData.price) || 0;
  const prompt = `بناءً على البيانات التالية لـ ${symbol}، قدّم توقعاً احتمالياً دقيقاً:
...
async function generateTradingPlan(symbol, analysis, portfolioSize = 10000) {
  ...
  const prompt = `ضع خطة تداول احترافية كاملة لـ ${symbol} (Spot):
...
async function analyzeMacroCorrelation(onChainData) {
  const system = `أنت "الفهد 🐆" — محلل اقتصادي كلي متخصص في ارتباط الكريبتو بالسياسة النقدية. اكتب بالعربية.`;
  const fg = onChainData?.fearGreed;
  const dom = onChainData?.btcDominance;
  const prompt = `حلل ارتباط BTC بالعوامل الكلية الحالية:
...
async function generateQuantAnalysis(symbol, backtest, mtf) {
  const system = `أنت "الفهد 🐆" — محلل كمي متخصص. قدّم إحصائيات متقدمة بالعربية.`;
...
- الحكم: ${backtest?.verdictAr || 'غير محدد'}

MTF: ${mtf?.dominantTrend || 'محايد'} | تناسق: ${mtf?.alignment ? (mtf.alignment*100).toFixed(0)+'%' : 'N/A'}

${hasData ? 'احسب وقدّم:' : 'البيانات محدودة — قدّم تقييماً نوعياً:'}
1. Expected Value (EV) لكل صفقة
2. Sharpe Ratio تقريبي
3. Maximum Drawdown المتوقع
4. احتمال الربح في الشهر القادم
5. Alpha مقارنةً بـ BTC+ETH
6. هل للإشارة ميزة إحصائية حقيقية؟ (Statistical Edge)`;
  return await callClaude(HAIKU, system, prompt);
}
