...
function cleanMarkdown(text) {
  ...
  return text
    ...
    .replace(/\*\*([^*]+)\*\*/g, '$1')
    .replace(/\*([^*]+)\*/g, '$1')
    .replace(/`{3}[\s\S]*?`{3}/g, function(m) {
      return m.replace(/`{3}[a-z]*\n?/g, '').replace(/`{3}/g, '');
    })
    .replace(/`([^`]+)`/g, '$1')
    .replace(/^[-*]\s+/gm, '• ')
    .replace(/\n{3,}/g, '\n\n')
...
async function analyzeSmartMoney(symbol, marketData) {
  const system = `أنت "الفهد 🐆" — محلل Smart Money Concepts متخصص بمستوى صندوق تحوط.
التاريخ: ${getCurrentDate()}. اكتب بالعربية. لا ## ولا ** ولا جداول.
عند نقص البيانات: استخدم Price Structure وهيمنة BTC والسياق الكلي للتحليل.`;
  const p = parseFloat(marketData.price) || 0;
  const mtf = marketData.mtf || {};
  const onChain = marketData.onChain || {};
