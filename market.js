...
async function getKlines(symbol, timeframe) {
  ...
  try {
    ...
    const resp = await axios.get(
      ...
      { params: geckoParams, timeout: 15000 }
    );
...
async function getHistoricalDaily(symbol, days = 1095) {
  ...
  try {
    const histParams = { vs_currency: 'usd', days, interval: 'daily' };
    if (process.env.COINGECKO_API_KEY) histParams.x_cg_demo_api_key = process.env.COINGECKO_API_KEY;
    const resp = await axios.get(
      `https://api.coingecko.com/api/v3/coins/${geckoId}/market_chart`,
      { params: histParams, timeout: 25000 }
    );
...
async function runBacktest(symbol, signalType, confidence) {
  ...
  try {
    ...
    // === الاستراتيجية: EMA Crossover + RSI + Volume ===
    for (let i = 50; i < closes.length - 15; i++) {
      const window = closes.slice(Math.max(0, i - 50), i);
      if (window.length < 30) continue;

      const rsi = calculateRSI(window);
      const ema20 = calculateEMA(window, 20);
      const ema50 = calculateEMA(window, Math.min(50, window.length));
      const bb = calculateBollingerBands(window);
      const atr = calculateATR(klines.slice(Math.max(0, i-14), i));
      ...
      const longSignal = signalType === 'long' &&
        ...
        ema20 > ema50 &&
        rsi > 40 && rsi < 68 &&
        bb.position > 0.3 && bb.position < 0.85;

      const shortSignal = signalType === 'short' &&
        closes[i] < ema20 &&
        ema20 < ema50 &&
        rsi > 32 && rsi < 60 &&
        bb.position < 0.7 && bb.position > 0.15;

      if (!longSignal && !shortSignal) continue;

      totalSignals++;
      const entry = closes[i];
      const future = closes.slice(i + 1, i + 16); // 15 شمعة للأمام
      ...
      if (outcome > 0) {
        wins++;
        currentLosses = 0;
        totalReturn += Math.min(outcome, targetPct * 100);
      } else {
        losses++;
        currentLosses++;
        maxConsecLosses = Math.max(maxConsecLosses, currentLosses);
        totalReturn += outcome;
        worstDD = Math.min(worstDD, outcome);
      }
    }
