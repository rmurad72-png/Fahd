...
const geckoQueue = { queue: [], processing: false, lastCall: 0, minInterval: 2500 };
...
async function getKlines(symbol, timeframe) {
  ...
  try {
    ...
    logger.warn(`🐆 Klines فشل ${symbol} ${timeframe} days=${days}: status=${error.response?.status} ${error.message}`);
    return [];
  }
}
...
function calculateConfidence(coin, mtf, onChain, type) {
  ...
  // تناسق الاتجاه (15)
  maxScore += 15;
  if ((c24raw > 0 && c7d > 0) || (c24raw < 0 && c7d < 0)) score += c24 > 3 ? 15 : 10;
  else score += 3;
