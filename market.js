  ...
  try {
    const response = await axios.get(
      'https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest',
      {
        ...
        params: { limit: 200, convert: 'USD', sort: 'market_cap', sort_dir: 'desc' },
        timeout: 12000
      }
    );
    const filtered = response.data.data
      .filter(function(c) {
        ...
        const vol = c.quote && c.quote.USD && c.quote.USD.volume_24h || 0;
...
async function getVerifiedPrice(symbol) {
  ...
  const quote = symbol.includes('/BTC') ? 'BTC' : 'USDT';
  const instId = base + '-' + quote;

  // OKX أولاً — أسرع وأدق للسعر الحالي
  try {
    const okxResp = await axios.get('https://www.okx.com/api/v5/market/ticker', {
      params: { instId: instId }, timeout: 8000
    });
    const okxData = okxResp.data && okxResp.data.data && okxResp.data.data[0];
    if (okxData && parseFloat(okxData.last) > 0) {
      const price = parseFloat(okxData.last);
      const open24h = parseFloat(okxData.open24h) || price;
      const change24h = open24h > 0 ? ((price - open24h) / open24h * 100) : 0;
      const result = { price, change24h, volume24h: parseFloat(okxData.vol24h)||0, symbol, source: 'OKX', verified: true };
      await setCache(cacheKey, result, 30);
      return result;
    }
  } catch(e) { logger.debug('getVerifiedPrice OKX: ' + e.message); }
  ...
  try {
    ...
    if (coins) {
      ...
      if (coin?.price > 0) {
        const result = {
          ...
          symbol, source: 'CMC_cache', verified: true
        };
    ...
    const result = { price, change24h: 0, symbol, source: 'CMC_direct', verified: true };
    await setCache(cacheKey, result, 60);
    return result;
  } catch (error) {
...
function analyzeTimeframe(klines) {
  ...
  return { trend, rsi: rsi || 50, signal: signal || 0, ema20: ema20 || current, ema50: ema50 || current, current, support: support || current * 0.95, resistance: resistance || current * 1.05, macd: macd || {}, bb: bb || {}, atr: atr || 0, zScore: zScore || 0, zInterpret: zInterpret || { signal: 'N/A', emoji: '' } };
}
...
async function getOKXKlines(symbol, interval, limit) {
  ...
  var base, quote;
  if (symbol.includes('/BTC') || symbol.endsWith('BTC') && !symbol.includes('USDT')) {
    base = symbol.replace('/BTC','').replace('BTC','').toUpperCase();
    if (!base) base = 'ETH';
    quote = 'BTC';
  } else {
    base = symbol.replace('/USDT','').replace('USDT','').toUpperCase();
    quote = 'USDT';
  }
  const instId = base + '-' + quote;
...
async function scanMarket(type = 'daily') {
  ...
  const opportunities = [];
  const batchSize = 10; // OKX/Binance بدون geckoQueue — نرفع التوازي
  ...
  // إضافة أزواج BTC/ETH للعملات الكبيرة (Top 20)
  const btcEthCoins = coins.slice(0, 20).filter(c => !['BTC','ETH','BNB'].includes(c.symbol));
  for (const coin of btcEthCoins.slice(0, 5)) {
    try {
      const mtfBtc = await getMTFAnalysis(coin.symbol + '/BTC', type).catch(() => null);
      if (mtfBtc && mtfBtc.alignment >= 0.8) {
        const confBtc = calculateConfidence(coin, mtfBtc, onChain, type);
        if (confBtc >= 60) {
          const levBtc = { target: coin.price * (type === 'daily' ? 1.10 : 1.22), stopLoss: coin.price * (type === 'daily' ? 0.97 : 0.93) };
          opportunities.push({
            symbol: coin.symbol + '/BTC', name: coin.name, price: coin.price,
            direction: 'long', confidence: confBtc, scanConfidence: confBtc,
            target: levBtc.target, stopLoss: levBtc.stopLoss,
            riskReward: Math.abs(levBtc.target - coin.price) / Math.max(Math.abs(coin.price - levBtc.stopLoss), 0.0001),
            mtfAlignment: mtfBtc.alignment || 0, market: 'spot', quote: 'BTC'
          });
        }
      }
    } catch(e) { /* skip */ }
  ...
  const result = {
    opportunities: opportunities.filter(o => o.confidence >= 60),
...
async function analyzeOpportunity(coin, type, onChain) {
  try {
    ...
    const rr = Math.abs(target - coin.price) / Math.max(Math.abs(coin.price - stopLoss), 0.0001);
    // فلتر R/R — لا تعرض فرصة إذا R/R أقل من 1.5:1
    if (rr < 1.5) {
      logger.debug(`🐆 رفض ${coin.symbol}: R/R ${rr.toFixed(2)} < 1.5`);
      return null;
    }
...
// ==================== PRICE HISTORY ====================
async function saveDailyPrices(coins) {
  ...
  for (const coin of coins) {
    try {
      ...
      await PriceHistory.findOneAndUpdate(
        { symbol: coin.symbol, date: today },
        {
          symbol: coin.symbol, date: today,
          open: price, high: price * (1 + Math.abs(parseFloat(coin.change24h)||0)/200),
          low: price * (1 - Math.abs(parseFloat(coin.change24h)||0)/200),
          close: price, volume24h: coin.volume24h || 0,
          marketCap: coin.marketCap || 0, change24h: parseFloat(coin.change24h)||0,
          timestamp: new Date()
        },
        { upsert: true, new: true }
      );
      saved++;
    } catch (e) { /* تجاهل التكرار */ }
  }
  logger.info('🐆 PriceHistory: حُفظ ' + saved + ' عملة ليوم ' + today);
  return saved;
}

async function getPriceHistoryFromDB(symbol, days) {
  days = days || 365;
  const { PriceHistory } = require('./database');
  const base = symbol.replace('/USDT', '').toUpperCase();
  try {
    const records = await PriceHistory.find({ symbol: base }).sort({ date: 1 }).limit(days).lean();
    if (records.length < 5) return [];
    return records.map(function(r) {
      return { time: new Date(r.date).getTime(), open: r.open||r.close, high: r.high||r.close, low: r.low||r.close, close: r.close, volume: r.volume24h||0 };
    });
  } catch (e) { return []; }
}
...
async function getPriceHistoryStats() {
  ...
  try {
    ...
    const symbols = await PriceHistory.distinct('symbol');
    const dates = await PriceHistory.distinct('date');
    const sorted = dates.sort();
    return {
      totalRecords, totalSymbols: symbols.length, totalDays: dates.length,
      oldestDate: sorted[0], latestDate: sorted[sorted.length-1],
      coverage: dates.length > 0 ? dates.length + ' يوم (' + sorted[0] + ' → ' + sorted[sorted.length-1] + ')' : 'لا توجد بيانات'
    };
  } catch (e) { return null; }
}
...
async function getPerformanceBenchmarks() {
  try {
    const [btc, eth] = await Promise.all([
      getVerifiedPrice('BTC/USDT'),
      getVerifiedPrice('ETH/USDT')
    ]);
    return {
      BTC: { price: btc.price, change24h: btc.change24h || 0 },
      ETH: { price: eth.price, change24h: eth.change24h || 0 }
    };
  } catch (e) { return null; }
}
...
async function getFearGreedIndex() {
  ...
  try {
    ...
    const result = {
      value: val, classification: d.value_classification,
      classificationAr: val >= 75 ? 'طمع شديد' : val >= 55 ? 'طمع' : val >= 45 ? 'محايد' : val >= 25 ? 'خوف' : 'خوف شديد',
      signal: val <= 25 ? 'فرصة شراء' : val >= 75 ? 'تحذير بيع' : 'محايد'
    };
    await setCache(cacheKey, result, 3600);
    return result;
  } catch (e) {
    logger.debug('FearGreed: ' + e.message);
    return null;
  }
}
...
async function getBTCDominance() {
  ...
  try {
    const coins = await getCached('top100_coins_v2') || [];
    const btc = coins.find(function(c) { return c.symbol === 'BTC'; });
...
async function getFundingRate(symbol) {
  try {
    const base = (symbol || 'BTC').replace('/USDT', '');
    const resp = await axios.get('https://www.okx.com/api/v5/public/funding-rate', {
      params: { instId: base + '-USDT-SWAP' }, timeout: 8000
    });
    const data = resp.data && resp.data.data && resp.data.data[0];
    return data ? parseFloat(data.fundingRate || 0) * 100 : 0;
  } catch (e) { return 0; }
}
...
async function getBTCOnChainMetrics() {
  ...
  if (cached) return cached;
  try {
    const resp = await axios.get('https://blockchain.info/stats?format=json', { timeout: 10000 });
    const d = resp.data;
    const result = {
      activeAddresses: d.n_unique_addresses || 0,
      hashRate: d.hash_rate || 0,
      mempoolSize: d.mempool_size || 0
    };
    await setCache(cacheKey, result, 7200);
    return result;
  } catch (e) { return { activeAddresses: 0, hashRate: 0, mempoolSize: 0 }; }
}
