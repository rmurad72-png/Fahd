...
async function start() {
  ...
  // حذف أي webhook قديم لضمان polling نظيف
  try {
    await bot.deleteWebHook();
    logger.info('🐆 Webhook محذوف — polling نظيف');
  } catch (e) {
    logger.debug('deleteWebhook:', e.message);
  }
  try {
...
async function handle(msg, fn) {
  // تحويل نص الأمر لـ lowercase لضمان case-insensitive
...
// مسح الأسواق
// ==================== BACKGROUND SCAN ====================
// Cache مشترك للمسح الخلفي — يُحدَّث كل 20 دقيقة (بدون إرسال للمستخدمين)
...
async function runBackgroundScan(type = 'daily') {
  ...
  try {
    ...
    // ✅ لا إرسال للمستخدمين هنا — Cache فقط
  } catch (e) {
...
bot.onText(/\/autotrade|التداول الالي/, async (msg) => {
  await handle(msg, async () => {
    ...
    await bot.sendMessage(msg.chat.id,
      `الفهد — التداول الالي Spot\n\nاليومي: ${s.autoTradeDaily ? 'فعال ✅' : 'موقف ⏸️'}\nالشهري: ${s.autoTradeMonthly ? 'فعال ✅' : 'موقف ⏸️'}\n\n⏰ جولات التنفيذ التلقائي (كل 4 ساعات):\n• 09:00 KSA (06:00 UTC)\n• 13:15 KSA (10:15 UTC)\n• 17:00 KSA (14:00 UTC)\n• 21:00 KSA (18:00 UTC)\n\nالتنفيذ يشترط استيفاء جميع فلاتر الاستراتيجية`,
      { reply_markup: { inline_keyboard: [
        [
...
// سجل الصفقات
// /dbhis — إحصائيات قاعدة البيانات التاريخية
bot.onText(/\/dbhis/, async (msg) => {
  await handle(msg, async () => {
    const loadMsg = await bot.sendMessage(msg.chat.id, '🐆 الفهد يجلب إحصائيات قاعدة البيانات...');
    try {
      const stats = await getPriceHistoryStats();
      await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
      if (!stats) { await bot.sendMessage(msg.chat.id, '🐆 لا توجد بيانات تاريخية بعد'); return; }
      let msg2 = '🐆 الفهد — قاعدة البيانات التاريخية\n━━━━━━━━━━━━━━━━━━━━\n\n';
      msg2 += '📊 إحصائيات Backtest Database:\n';
      msg2 += '   العملات المحفوظة: ' + (stats.totalSymbols || 0) + '\n';
      msg2 += '   إجمالي السجلات: ' + (stats.totalRecords || 0).toLocaleString() + '\n';
      msg2 += '   أقدم تاريخ: ' + (stats.oldestDate || 'N/A') + '\n';
      msg2 += '   أحدث تاريخ: ' + (stats.newestDate || 'N/A') + '\n';
      msg2 += '   سنوات البيانات: ' + (stats.dataYears?.toFixed(1) || 'N/A') + '\n\n';
      msg2 += '💡 ملاحظة: تُحفظ يومياً عند 00:05 UTC\n';
      msg2 += '⚠️ تحليل الفهد — ليس نصيحة مالية';
      await bot.sendMessage(msg.chat.id, msg2);
    } catch(e) {
      await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
      await bot.sendMessage(msg.chat.id, '🐆 خطأ في جلب إحصائيات قاعدة البيانات: ' + e.message);
    }
  });
});
...
bot.on('callback_query', async (query) => {
  ...
  try {
    ...
    if (data === 'show_more_opps') {
      ...
      extraMsg += '⚠️ تحليل الفهد — ليس نصيحة مالية';
      const extraButtons = extras.slice(0, 5).map(o => ([{
        text: 'تحليل ' + o.symbol + ' (' + o.confidence + '%)',
        callback_data: 'analyze_' + o.symbol.replace('/USDT','').replace('/','')
      }]));
    ...
    if (data.startsWith('manual_execute_')) {
      ...
      const loadMsg = await bot.sendMessage(chatId, '🐆 الفهد يحلل ' + sym + '/USDT قبل التنفيذ...');
      try {
        const user = await User.findOne({ telegramId: userId });
        if (!user) throw new Error('مستخدم غير موجود');

        // جلب السعر الحي أولاً للتحقق
        let livePrice = 0;
        try {
          const pd = await getVerifiedPrice(sym + '/USDT');
          livePrice = pd?.price || 0;
        } catch(e) { logger.debug('livePrice manual:', e.message); }

        if (!livePrice) {
          await bot.deleteMessage(chatId, loadMsg.message_id).catch(() => {});
          await bot.sendMessage(chatId, '🐆 الفهد — لا يوجد سعر لـ ' + sym + ' حالياً\nحاول مجدداً بعد دقيقة');
          await bot.answerCallbackQuery(query.id);
          return;
        }
        ...
        if (!opp) {
          opp = { symbol: sym + '/USDT', direction: 'long', confidence: conf, price: livePrice, mtfAlignment: 1 };
        ...
        if (result.executed > 0) {
          ...
          // رسالة رفض مفصّلة
          const reason = result.reason || 'لا تستوفي المعايير الحالية';
          const rejDetails = result.rejectionSummary ? '\n\n🔍 تفاصيل:\n' + result.rejectionSummary : '';
          const isWait = reason.includes('wait') || reason.includes('انتظار');
          
          if (isWait) {
            // التحليل يوصي بالانتظار — اعطِ المستخدم خيار التجاوز
            await bot.sendMessage(chatId,
              '🐆 الفهد — ' + sym + '/USDT\n\n⏸️ التوصية: انتظر\n' + reason + rejDetails +
              '\n\nهل تريد التجاوز وتنفيذ الصفقة رغم التحذير؟',
              { reply_markup: { inline_keyboard: [[
                { text: '⚡ نعم، نفّذ رغم التحذير', callback_data: 'force_execute_' + sym + '_' + conf },
                { text: '❌ لا، أتبع توصية الفهد', callback_data: 'dismiss_execute' }
              ]]}}
            );
          } else {
            await bot.sendMessage(chatId,
              '🐆 الفهد — لم يُنفذ ' + sym + '/USDT\n\nالسبب: ' + safe(reason) + rejDetails
            );
          }
        }
      } catch(e) {
        logger.error('manual_execute error:', e.message);
        await bot.deleteMessage(chatId, loadMsg.message_id).catch(() => {});
        // معالجة خطأ 400 من Anthropic API
        if (e.message && (e.message.includes('400') || e.message.includes('Request failed'))) {
          await bot.sendMessage(chatId,
            '🐆 الفهد — تعذّر إكمال التحليل لـ ' + sym + '\n\nالسبب: بيانات غير كافية حالياً\nحاول مجدداً بعد دقيقتين'
          );
        } else {
          await bot.sendMessage(chatId, '🐆 الفهد — فشل التنفيذ: ' + safe(e.message));
        }
      }
      await bot.answerCallbackQuery(query.id);
      return;
    ...
    if (data.startsWith('force_execute_')) {
      ...
      try {
        const freshScan = await scanMarket('daily');
        const liveP = await getVerifiedPrice(sym + '/USDT').catch(() => ({ price: 0 }));
        let opp = (freshScan.opportunities || []).find(o => o.symbol === sym + '/USDT');
        if (!opp) {
          opp = { symbol: sym + '/USDT', direction: 'long', confidence: conf, price: liveP.price, mtfAlignment: 0.8 };
          freshScan.opportunities = [opp, ...(freshScan.opportunities || [])];
        }
        const result = await executeTradesFromScan(userId, freshScan, 'daily', true);
        await bot.deleteMessage(chatId, loadMsg.message_id).catch(() => {});
        if (result.executed > 0) {
          ...
          await bot.sendMessage(chatId, '🐆 الفهد — لم يُنفذ ' + sym + '\nالسبب: ' + safe(result.reason || 'لا تستوفي المعايير'));
        }
      } catch(e) {
        await bot.deleteMessage(chatId, loadMsg.message_id).catch(() => {});
        await bot.sendMessage(chatId, '🐆 فشل التنفيذ: ' + safe(e.message));
      }
      await bot.answerCallbackQuery(query.id);
      return;
    }

    if (data === 'dismiss_execute') {
      await bot.answerCallbackQuery(query.id, { text: '✅ تم — الفهد يراقب الفرصة الأفضل' });
      return;
    }
...
bot.on('polling_error', (err) => {
  ...
  // معالجة 409 Conflict — نسختان تعملان في نفس الوقت
  if (err.message && err.message.includes('409')) {
    logger.error('🔴 409 Conflict — إيقاف هذه النسخة لمنع التعارض');
...
bot.onText(/\/smartmoney(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendSequence(bot, msg.chat.id, fmtLongAnalysis('Smart Money Concepts', '🪙 ' + symbol, analysis || ''));
  });
});
