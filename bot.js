  ...
  try {
    ...
    if (data.startsWith('quant_')) {
      ...
      try {
        ...
        await bot.deleteMessage(userId, loadMsg.message_id).catch(()=>{});
        await sendLong(bot, userId, `🐆 الفهد — Quant\n${DIVIDER}\n📊 ${sym}\n\n${safe(analysis, 8000)}\n\n⚠️ ليس نصيحة مالية`);
    ...
    if (data.startsWith('forecast_')) {
      ...
      try {
        ...
        await bot.deleteMessage(userId, loadMsg.message_id).catch(()=>{});
        await sendLong(bot, userId, `🐆 الفهد — AI Forecast\n${DIVIDER}\n🪙 ${sym}\n\n${safe(forecast, 8000)}\n\n⚠️ نموذج احتمالي`);
    ...
    if (data.startsWith('smartmoney_')) {
      ...
      try {
        ...
        await bot.deleteMessage(userId, loadMsg.message_id).catch(()=>{});
        await sendLong(bot, userId, `🐆 الفهد — Smart Money\n${DIVIDER}\n🪙 ${sym}\n\n${safe(analysis, 8000)}\n\n⚠️ ليس نصيحة مالية`);
    ...
    if (data === 'macro_analysis') {
      ...
      try {
        ...
        await bot.deleteMessage(userId, loadMsg.message_id).catch(()=>{});
        await sendLong(bot, userId, `🐆 الفهد — تحليل كلي\n${DIVIDER}\n🏦 BTC + الفيدرالي + M2\n\n${safe(analysis, 8000)}\n\n⚠️ ليس نصيحة مالية`);
      } catch(e) { await bot.editMessageText('فشل التحليل', {chat_id: userId, message_id: loadMsg.message_id}).catch(()=>{}); }
    }
...
bot.onText(/\/smartmoney(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    await sendLong(bot, msg.chat.id, `🐆 الفهد — Smart Money Concepts\n${DIVIDER}\n🪙 ${symbol}\n\n${safe(analysis, 8000)}\n\n${DIVIDER}\n⚠️ تحليل الفهد — ليس نصيحة مالية`);
  });
});
...
bot.onText(/\/forecast(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendLong(bot, msg.chat.id, `🐆 الفهد — AI Forecast\n${DIVIDER}\n🪙 ${symbol}\n\n${safe(forecast, 8000)}\n\n${DIVIDER}\n⚠️ نموذج احتمالي — ليس ضماناً`);
  });
});
...
bot.onText(/\/macro/, async (msg) => {
  await handle(msg, async () => {
    ...
    const analysis = await analyzeMacroCorrelation(onChain);
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendLong(bot, msg.chat.id, `🐆 الفهد — تحليل كلي\n${DIVIDER}\n🏦 BTC + الفيدرالي + السيولة العالمية\n\n${safe(analysis, 8000)}\n\n${DIVIDER}\n⚠️ تحليل الفهد — ليس نصيحة مالية`);
  });
});
...
bot.onText(/\/quant(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendLong(bot, msg.chat.id, `🐆 الفهد — Quant Analysis\n${DIVIDER}\n📊 ${symbol}\n\n${safe(analysis, 8000)}\n\n${DIVIDER}\n⚠️ تحليل الفهد — ليس نصيحة مالية`);
  });
});
...
bot.onText(/\/plan(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendLong(bot, msg.chat.id, `🐆 الفهد — خطة تداول احترافية\n${DIVIDER}\n🪙 ${symbol} | محفظة: $${portfolio.toFixed(0)}\n\n${plan || 'لا توجد بيانات'}\n\n${DIVIDER}\n⚠️ تحليل الفهد — ليس نصيحة مالية`);
  });
});
...
bot.onText(/\/monthly/, async (msg) => {
  await handle(msg, async () => {
    ...
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendLong(bot, msg.chat.id, `🐆 الفهد — المخطط الشهري\n${DIVIDER}\n📅 ${new Date().toLocaleDateString('ar-SA', {month:'long', year:'numeric'})}\n💼 المحفظة: $${portfolio.toFixed(0)}\n\n${plan || 'لا توجد بيانات'}\n\n${DIVIDER}\n⚠️ تحليل الفهد — ليس نصيحة مالية`);
  });
});
