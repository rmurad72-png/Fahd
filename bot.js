...
bot.onText(/\/plan(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    const plnMsgs = fmtLongAnalysis('خطة تداول احترافية', '🪙 ' + symbol + ' | محفظة: $' + portfolio.toFixed(0), plan || '');
...
bot.onText(/\/monthly/, async (msg) => {
  await handle(msg, async () => {
    ...
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    const mnthHeader = '📅 ' + new Date().toLocaleDateString('ar-SA', {month:'long', year:'numeric'}) + ' | 💼 $' + portfolio.toFixed(0);
  ...
  await handle(msg, async () => {
    ...
    if (!closed.length) {
      ...
      closed.forEach((t, i) => {
        ...
        totalPnl += pnl;
        if (pnl >= 0) wins++;
        closedMsg += `${i+1}. ${emoji} ${t.symbol} ${pnl >= 0 ? '+' : ''}${pnl.toFixed(2)}%\n`;
