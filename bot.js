...
const { securityMiddleware, isAdmin, banUser, unbanUser, sanitizeForClaude, getSecurityReport } = require('./security');
const { scanMarket, getVerifiedPrice, getTopCoins, getMTFAnalysis, runBacktest, getHistoricalDaily, getFullOnChainData, getPerformanceBenchmarks, getPriceHistoryStats, saveDailyPrices } = require('./market');
...
const DIVIDER = '━━━━━━━━━━━━━━━━━━━━';
const MAX_MSG = 3800; // حد تيليغرام الآمن

// إرسال رسالة طويلة مقسّمة تلقائياً
async function sendLong(bot, chatId, text, options = {}) {
  if (text.length <= MAX_MSG) {
    return bot.sendMessage(chatId, text, options);
  }
  // تقسيم عند السطر الجديد لتجنب قطع الجمل
  const parts = [];
  let current = '';
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    lastMsg = await bot.sendMessage(chatId, prefix + parts[i], isLast ? options : {});
...
function parseSymbol(input) {
  ...
  var sym = String(input).toUpperCase().trim().replace(/\s+/g, '');
  if (sym.endsWith('BTC') && !sym.includes('/') && sym.length > 3) {
    var base = sym.slice(0, -3);
    if (base.length >= 2 && base !== 'USD') return base + '/BTC';
  }
  if (!sym.includes('/') && !sym.endsWith('USDT') && !sym.endsWith('BTC')) {
    return sym + '/USDT';
  }
  return sym;
}
...
// خزن الفرص الإضافية مؤقتاً لكل chat (10 دقائق)
const pendingScanOpps = new Map();
setBot(bot);
  ...
  setTimeout(async () => {
    try {
      ...
      const r2 = await User.updateMany(
        { 'settings.confidenceThreshold': { $gt: 60 } },
        { $set: { 'settings.confidenceThreshold': 60 } }
      );
...
// مسح الأسواق
// ==================== BACKGROUND SCAN ====================
// Cache مشترك للمسح الخلفي — يُحدَّث كل 20 دقيقة
let backgroundScanCache = { daily: null, monthly: null };
let backgroundScanRunning = false;

async function runBackgroundScan(type = 'daily') {
  if (backgroundScanRunning) return;
  backgroundScanRunning = true;
  try {
    const result = await scanMarket(type);
    backgroundScanCache[type] = { result, timestamp: Date.now() };
    logger.info(`🐆 Background Scan: ${result.opportunities?.length || 0} فرصة (${type})`);
  } catch (e) {
    logger.warn(`🐆 Background Scan خطأ: ${e.message}`);
  } finally {
    backgroundScanRunning = false;
  }
}

// تشغيل أولي بعد 30 ثانية من البدء
setTimeout(() => runBackgroundScan('daily'), 30000);
// تحديث كل 20 دقيقة
setInterval(() => runBackgroundScan('daily'), 20 * 60 * 1000);
...
bot.onText(/\/scan|مسح الاسواق/, async (msg) => {
  await handle(msg, async () => {
    ...
    const extraOpps = opportunities.length - top5opps.length;
    const analyzeButtons = top5opps.map(o => ([{
    ...
    const actionButtons = [];
    if (extraOpps > 0) {
      actionButtons.push({ text: `✅ عرض ${extraOpps} فرصة إضافية`, callback_data: 'show_more_opps' });
      actionButtons.push({ text: '❌ لا، يكفي', callback_data: 'dismiss_more_opps' });
    }
    actionButtons.push({ text: '⚡ تنفيذ الصفقات الآن', callback_data: 'execute_now' });

    const keyboard = opportunities.length > 0 ? {
      inline_keyboard: [
        ...analyzeButtons,
        actionButtons.length > 1 ? [actionButtons[0], actionButtons[1]] : [],
...
bot.onText(/\/analyze(?:\s+(\S+))?|تحليل عميق/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    if (analysis.recommendation === 'long' || analysis.recommendation === 'short') {
      analyzeButtons.push([{
        text: `${analysis.recommendation === 'long' ? '📈 شراء' : '📉 بيع'} ${symbol} (توصية الفهد)`,
        callback_data: `open_${symbol}_${analysis.recommendation}_${analysis.confidence}`
      }]);
    }
    // زر التنفيذ اليدوي يظهر دائماً ما لم تكن التوصية avoid
    if (analysis.recommendation !== 'avoid') {
      analyzeButtons.push([{
        text: `⚡ تنفيذ ${symbol} يدوياً الآن`,
        callback_data: `manual_execute_${symbol}_${analysis.confidence}`
      }]);
    }
    await bot.sendMessage(msg.chat.id, formatAnalysis(analysis, symbol), {
      reply_markup: analyzeButtons.length > 0 ? { inline_keyboard: analyzeButtons } : undefined
    });
  });
});
...
bot.onText(/\/funds\s+([\d.]+)/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    await bot.sendMessage(msg.chat.id, `الفهد — تم اضافة الرصيد\n\nالمبلغ: $${(amount || 0).toFixed(2)}\nالرصيد الجديد: $${(balance || 0).toFixed(2)}`);
  });
});
...
bot.onText(/\/ratings/, async (msg) => {
  await handle(msg, async () => {
    ...
    txt += `متوسط التقييم: ${stars} (${(avgRating || 0).toFixed(1)}/5)\n`;
...
// /performance — تقرير الأداء الشامل
bot.onText(/\/performance|أداء التداول/, async (msg) => {
  await handle(msg, async () => {
    const userId = String(msg.from.id);
    const metrics = await getPerformanceMetrics(userId);
    const user = await require('./database').User.findOne({ telegramId: userId });
    if (!metrics) {
      await bot.sendMessage(msg.chat.id, '🐆 لا توجد صفقات مغلقة خلال 30 يوم بعد');
      return;
    }
    const strictStatus = user?.settings?.strictMode ? '⚠️ وضع التشديد نشط' : '✅ وضع عادي';
    let perfMsg = `🐆 الفهد — تقرير الأداء (30 يوم)
━━━━━━━━━━━━━━━━━━━━

`;
    perfMsg += `${strictStatus}

`;
    perfMsg += `📊 الإحصائيات:
`;
    perfMsg += `   الصفقات: ${metrics.trades} | ✅ ${metrics.wins} | ❌ ${metrics.losses}
`;
    perfMsg += `   Win Rate: ${metrics.winRate}%
`;
    perfMsg += `   Profit Factor: ${metrics.profitFactor}
`;
    perfMsg += `   Expectancy: ${metrics.expectancy}
`;
    perfMsg += `   PnL الإجمالي: ${metrics.totalPnL >= 0 ? '+' : ''}${metrics.totalPnL.toFixed(2)}

`;
    perfMsg += `⚙️ الإعدادات الحالية:
`;
    perfMsg += `   حد الثقة: ${user?.settings?.confidenceThreshold || 60}%
`;
    perfMsg += `   Risk per trade: ${user?.settings?.dailyRiskPercent || 1}%
`;
    perfMsg += `   أزواج: USDT + BTC + ETH

`;
    perfMsg += `⚠️ تحليل الفهد — ليس نصيحة مالية`;
    await bot.sendMessage(msg.chat.id, perfMsg);
  });
});
...
bot.on('callback_query', async (query) => {
  ...
  try {
    ...
    if (data === 'show_more_opps') {
      ...
      let extraMsg = '🐆 الفهد — الفرص الإضافية\n━━━━━━━━━━━━━━━━━━━━\n\n';
      extras.forEach((o, i) => {
        const dir = o.direction === 'long' ? '📈 شراء' : '📉 بيع';
        extraMsg += (i + 6) + '. ' + o.symbol + ' ' + dir + '\n';
        extraMsg += '   السعر: ' + (o.price||0).toFixed(4) + ' | ثقة: ' + o.confidence + '%\n';
        extraMsg += '   هدف: ' + (o.target||0).toFixed(4) + ' | وقف: ' + (o.stopLoss||0).toFixed(4) + '\n';
        extraMsg += '   R/R: ' + (o.riskReward||0).toFixed(1) + ':1 | MTF: ' + ((o.mtfAlignment||0)*100).toFixed(0) + '%\n\n';
      });
            // لا تكرار لـ "ليس نصيحة مالية" — موجودة في رسالة المسح الأصلي
      const extraButtons = [
        ...extras.slice(0, 5).map(o => ([{
          text: 'تحليل ' + o.symbol + ' (' + o.confidence + '%)',
          callback_data: 'analyze_' + o.symbol.replace('/USDT','').replace('/','')
        }])),
        [{ text: '⚡ تنفيذ الصفقات الآن', callback_data: 'execute_now' }]
      ];
    ...
    if (data === 'execute_now') {
      ...
      // التنفيذ اليدوي — isManual=true يتجاوز شرط autoTrade

      result = await executeTradesFromScan(userId, freshScan, 'daily', true); // isManual=true
      logger.info(`🐆 execute_now daily: ${result.executed} صفقة — ${result.reason || 'ok'}`);

      if (result.executed === 0) {
        // نجرب نفس فرص daily مع عتبة أقل بدلاً من monthly
        const lowerResult = await executeTradesFromScan(userId, freshScan, 'daily', true);
        if (lowerResult.executed > 0) { result = lowerResult; }
        // fallback لـ monthly فقط إذا لا فرص daily
        if (result.executed === 0) {
        const monthScan = await scanMarket('monthly');
        const monthResult = await executeTradesFromScan(userId, monthScan, 'daily', true); // daily risk بدلاً من monthly
        logger.info(`🐆 execute_now monthly: ${monthResult.executed} صفقة — ${monthResult.reason || 'ok'}`);
        if (monthResult.executed > 0) result = monthResult;
        }
      }
    ...
    // تنفيذ يدوي مباشر من التحليل العميق
    if (data.startsWith('manual_execute_')) {
      const parts = data.replace('manual_execute_', '').split('_');
      const sym = parts[0];
      const conf = parseInt(parts[1]) || 70;
      const loadMsg = await bot.sendMessage(chatId, '🐆 الفهد يُنفذ ' + sym + '/USDT يدوياً...');
      try {
        const user = await User.findOne({ telegramId: userId });
        if (!user) throw new Error('مستخدم غير موجود');
        const { executeTradesFromScan } = require('./autotrader');
        const { scanMarket } = require('./market');
        const freshScan = await scanMarket('daily');
        // البحث عن العملة في نتائج المسح أو إنشاء فرصة بسيطة
        let opp = (freshScan.opportunities || []).find(o => o.symbol === sym + '/USDT');
        if (!opp) {
          const { getVerifiedPrice } = require('./market');
          const priceData = await getVerifiedPrice(sym + '/USDT');
          opp = { symbol: sym + '/USDT', direction: 'long', confidence: conf, price: priceData.price, mtfAlignment: 1 };
          freshScan.opportunities = [opp, ...(freshScan.opportunities || [])];
        }
        const result = await executeTradesFromScan(userId, freshScan, 'daily', true);
        await bot.deleteMessage(chatId, loadMsg.message_id).catch(() => {});
        if (result.executed > 0) {
          const { formatTrade } = require('./formatters');
          for (const { trade } of result.trades) {
            await bot.sendMessage(chatId, formatTrade(trade, true));
          }
        } else {
          await bot.sendMessage(chatId, '🐆 الفهد — لم يُنفذ ' + sym + '/USDT\n\nالسبب: ' + (result.reason || 'لا تستوفي المعايير الحالية'));
        }
      } catch(e) {
        await bot.editMessageText('فشل التنفيذ: ' + e.message, { chat_id: chatId, message_id: loadMsg.message_id }).catch(() => {});
      }
      await bot.answerCallbackQuery(query.id);
      return;
    }
    ...
    if (data.startsWith('quant_')) {
      ...
      try {
        ...
        let qp1 = null;
      try { const qd = await getVerifiedPrice(sym + '/USDT'); qp1 = qd && qd.price; } catch(e) {}
      const analysis = await generateQuantAnalysis(sym, bt.value, mtf.value||{}, null, qp1);
        await bot.deleteMessage(userId, loadMsg.message_id).catch(()=>{});
        await sendSequence(bot, userId, fmtLongAnalysis('Quant Analysis', '📊 ' + sym, analysis || ''));
    ...
    if (data.startsWith('smartmoney_')) {
      ...
      try {
        ...
        await bot.deleteMessage(userId, loadMsg.message_id).catch(()=>{});
        await sendSequence(bot, userId, fmtLongAnalysis('Smart Money', '🪙 ' + sym, analysis || ''));
    ...
    if (data === 'macro_analysis') {
      ...
      try {
        ...
        const [macBench] = await Promise.allSettled([getPerformanceBenchmarks()]);
      const macBenchData = macBench.status === 'fulfilled' ? macBench.value : null;
      const analysis = await analyzeMacroCorrelation(onChain, macBenchData);
        await bot.deleteMessage(userId, loadMsg.message_id).catch(()=>{});
        await sendSequence(bot, userId, fmtLongAnalysis('تحليل كلي', '🏦 BTC + الفيدرالي + M2', analysis || ''));
      } catch(e) { await bot.editMessageText('فشل التحليل', {chat_id: userId, message_id: loadMsg.message_id}).catch(()=>{}); }
    }
...
// ==================== ADMIN ====================
// /admin_security — تقرير أمني
bot.onText(/\/admin_security/, async (msg) => {
  if (!isAdmin(msg.from.id)) return;
  const report = await getSecurityReport();
  if (!report) { await bot.sendMessage(msg.chat.id, 'فشل جلب التقرير'); return; }
  let txt = '🔐 تقرير أمني\n' + DIVIDER + '\n';
  txt += '🚫 محظورون دائماً: ' + report.banned + '\n';
  txt += '⏳ محظورون مؤقتاً: ' + report.activeTempBans + '\n';
  txt += '👁️ مراقَبون: ' + report.totalMonitored + '\n\n';
  if (report.topSuspicious?.length) {
    txt += 'الأكثر إثارة للشك:\n';
    report.topSuspicious.forEach((u, i) => {
      txt += (i+1) + '. ID:' + u.telegramId + ' — نقاط: ' + u['security']?.suspiciousCount + (u.isBanned ? ' 🚫' : '') + '\n';
    });
  }
  await bot.sendMessage(msg.chat.id, txt);
});
...
bot.onText(/\/smartmoney(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    const [coins, mtf, onChain, liveP] = await Promise.allSettled([
      getTopCoins(), getMTFAnalysis(symbol + '/USDT', 'daily'),
      getFullOnChainData(symbol), getVerifiedPrice(symbol + '/USDT')
    ]);
    const coinData = (coins.value||[]).find(c => c.symbol === symbol) || { symbol };
    const livePrice = liveP.status === 'fulfilled' ? liveP.value?.price : 0;
    const enriched = {
      ...coinData,
      price: livePrice > 0 ? livePrice : (coinData.price || 0),
      mtf: mtf.value || {},
      onChain: onChain.value || {}
    };
    const analysis = await analyzeSmartMoney(symbol, enriched);
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendSequence(bot, msg.chat.id, fmtLongAnalysis('Smart Money Concepts', '🪙 ' + symbol + '/USDT', analysis || ''));
  });
});
...
bot.onText(/\/forecast(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    const [coins, bt, mtf, livePrice] = await Promise.allSettled([
      getTopCoins(),
      runBacktest(symbol + '/USDT', 'long', 70),
      getMTFAnalysis(symbol + '/USDT', 'daily'),
      getVerifiedPrice(symbol + '/USDT') // جلب السعر الحي مباشرة
    ]);
    const coinData = (coins.value||[]).find(c => c.symbol === symbol) || { symbol };
    // دمج السعر الحي مع بيانات العملة — الأولوية للسعر الحي
    const livePriceData = livePrice.status === 'fulfilled' ? livePrice.value : null;
    const enrichedData = {
      ...coinData,
      price: (livePriceData?.price > 0 ? livePriceData.price : coinData.price) || 0,
      change24h: livePriceData?.change24h ?? coinData.change24h ?? 0,
      zScore: mtf.value?.zScore || 0,
      zInterpret: mtf.value?.zInterpret || ''
    };
    // إذا لا يوجد سعر — أبلغ المستخدم بوضوح
    if (!enrichedData.price || enrichedData.price === 0) {
      await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
      await bot.sendMessage(msg.chat.id, `🐆 الفهد — لم أتمكن من جلب سعر ${symbol} حالياً\nالسبب: البيانات غير متوفرة من المصادر\nحاول مجدداً بعد دقيقة`);
      return;
    }
    const forecast = await generateAIForecast(symbol, enrichedData, bt.value, mtf.value||{});
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    if (!forecast || forecast.trim().length < 50) {
      await bot.sendMessage(msg.chat.id, `🐆 الفهد — فشل بناء نموذج التوقع لـ ${symbol}\nحاول مجدداً`);
      return;
    }
    await sendSequence(bot, msg.chat.id, fmtLongAnalysis('AI Forecast', '🪙 ' + symbol + '/USDT', forecast || ''));
  });
});
...
bot.onText(/\/macro/, async (msg) => {
  await handle(msg, async () => {
    ...
    const [macBench] = await Promise.allSettled([getPerformanceBenchmarks()]);
      ...
      const analysis = await analyzeMacroCorrelation(onChain, macBenchData);
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendSequence(bot, msg.chat.id, fmtLongAnalysis('تحليل كلي', '🏦 BTC + الفيدرالي + السيولة العالمية', analysis || ''));
  });
});
...
bot.onText(/\/quant(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    let qp2 = null;
      try { const qd2 = await getVerifiedPrice(symbol + '/USDT'); qp2 = qd2 && qd2.price; } catch(e) {}
      const analysis = await generateQuantAnalysis(symbol, bt.value, mtf.value || {}, null, qp2);
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    await sendSequence(bot, msg.chat.id, fmtLongAnalysis('Quant Analysis', '📊 ' + symbol, analysis || ''));
  });
});
...
bot.onText(/\/plan(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    // فحص الزوج المدعوم — USDT/BTC/ETH فقط
    const rawSym = match[1] || '';
    const symUpper = rawSym.toUpperCase();
    const hasValidQuote = symUpper.endsWith('USDT') || symUpper.endsWith('BTC') || symUpper.endsWith('ETH') || (!symUpper.includes('/') && !symUpper.includes('USDT') && !symUpper.includes('BTC') && !symUpper.includes('ETH'));
    if (symUpper.includes('/') && !hasValidQuote) {
      await bot.sendMessage(msg.chat.id, '🐆 الفهد يدعم أزواج USDT و BTC و ETH فقط\nمثال: /plan BTC أو /plan ETHUSDT');
      return;
    }
    ...
    let planPrice = null;
      try { const pd = await getVerifiedPrice(symbol + '/USDT'); planPrice = pd && pd.price; } catch(e) {}
      const plan = await generateTradingPlan(symbol, deepResult, portfolio, planPrice);
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    const plnMsgs = fmtLongAnalysis('خطة تداول احترافية', '🪙 ' + symbol + ' | محفظة: $' + (portfolio || 0).toFixed(0), plan || '');
    await sendSequence(bot, msg.chat.id, plnMsgs);
  });
});
...
bot.onText(/\/monthly/, async (msg) => {
  await handle(msg, async () => {
    ...
    await bot.deleteMessage(msg.chat.id, loadMsg.message_id).catch(() => {});
    const mnthHeader = '📅 ' + new Date().toLocaleDateString('ar-SA', {month:'long', year:'numeric'}) + ' | 💼 $' + (portfolio || 0).toFixed(0);
    const mnthMsgs = fmtLongAnalysis('المخطط الشهري', mnthHeader, plan || '');
    await sendSequence(bot, msg.chat.id, mnthMsgs);
  });
});
...
bot.onText(/\/trades/, async (msg) => {
  await handle(msg, async () => {
    const { Trade } = require('./database');
    const userId = msg.chat.id.toString();
    const [open, closed] = await Promise.all([
      Trade.find({ userId, status: { $in: ['open', 'pending_entry'] } }).sort({ openedAt: -1 }).limit(10),
      Trade.find({ userId, status: 'closed' }).sort({ closedAt: -1 }).limit(10)
    ]);

    // بطاقة الصفقات المفتوحة
    let openMsg = `🐆 الفهد — الصفقات المفتوحة\n${DIVIDER}\n`;
    if (!open.length) {
      openMsg += '📭 لا توجد صفقات مفتوحة حالياً';
    } else {
      open.forEach((t, i) => {
        const dir = t.direction === 'long' ? '📈' : '📉';
        const days = Math.floor((Date.now() - new Date(t.openedAt)) / 86400000);
        openMsg += `${i+1}. ${dir} ${t.symbol}\n`;
        openMsg += `   دخول: $${t.entryPrice?.toFixed(4)} | هدف: $${t.targetPrice?.toFixed(4)}\n`;
        openMsg += `   وقف: $${t.stopLoss?.toFixed(4)} | منذ: ${days} يوم\n`;
        openMsg += `   الثقة: ${t.confidence}%\n`;
      });
    }

    // بطاقة الصفقات المغلقة
    let closedMsg = `🐆 الفهد — آخر الصفقات المغلقة\n${DIVIDER}\n`;
    if (!closed.length) {
      closedMsg += '📭 لا توجد صفقات مغلقة بعد';
    } else {
      let totalPnl = 0;
      let wins = 0;
      closed.forEach((t, i) => {
        ...
        totalPnl += pnl;
        if (pnl >= 0) wins++;
        closedMsg += `${i+1}. ${emoji} ${t.symbol} ${pnl >= 0 ? '+' : ''}${(pnl || 0).toFixed(2)}%\n`;
        closedMsg += `   السبب: ${t.closeReason || 'N/A'} | ${new Date(t.closedAt).toLocaleDateString('ar-SA')}\n`;
      });
      closedMsg += `${DIVIDER}\n`;
      closedMsg += `📊 الإجمالي: ${wins}/${closed.length} رابحة | متوسط: ${(totalPnl/closed.length).toFixed(2)}%`;
    }

    await bot.sendMessage(msg.chat.id, openMsg);
    await new Promise(r => setTimeout(r, 300));
    await bot.sendMessage(msg.chat.id, closedMsg);
  });
});
...
bot.onText(/\/admin_users/, async (msg) => {
  ...
  for (const u of users) {
    ...
    txt += `  رصيد: $${(u.portfolio && u.portfolio.balance || 0).toFixed(2)} | صفقات مفتوحة: ${openCount}\n`;
...
bot.onText(/\/admin_trades/, async (msg) => {
  ...
  trades.forEach((t, i) => {
    ...
    txt += `   دخول: $${(t.entryPrice || 0).toFixed(4)} | المستخدم: ${t.userId}\n\n`;
  });
  await bot.sendMessage(msg.chat.id, txt);
});
...
bot.onText(/\/admin_report/, async (msg) => {
  ...
  txt += `PnL الشهري: $${(monthPnL || 0).toFixed(2)}\n`;
