  ...
  setTimeout(async () => {
    try {
      ...
      const r2 = await User.updateMany(
        { 'settings.confidenceThreshold': { $gt: 70 } },
        { $set: { 'settings.confidenceThreshold': 65 } }
      );
...
bot.on('callback_query', async (query) => {
  ...
  try {
    ...
    if (data === 'execute_now') {
      ...
      // التنفيذ اليدوي يتجاوز شرط autoTrade دائماً
      const origDaily = user.settings.autoTradeDaily;
      const origMonthly = user.settings.autoTradeMonthly;
      user.settings.autoTradeDaily = true;
      user.settings.autoTradeMonthly = true;
      ...
      if (result.executed === 0) {
        ...
        const monthResult = await executeTradesFromScan(userId, monthScan, 'monthly');
      ...
      user.settings.autoTradeDaily = origDaily;
      user.settings.autoTradeMonthly = origMonthly;
    ...
    if (data.startsWith('quant_')) {
      ...
      try {
        ...
        const analysis = await generateQuantAnalysis(sym, bt.value, mtf.value||{}, null);
        await bot.deleteMessage(userId, loadMsg.message_id).catch(()=>{});
