...
const { openTrade, closeTrade, getPortfolioSnapshot, getPerformanceStats, addFunds, checkAutoStrictening, getPerformanceMetrics } = require('./portfolio');
...
async function start() {
  ...
  setTimeout(async () => {
    try {
      // توحيد confidenceThreshold — الجميع على 60% (الحد المعتمد)
      ...
      const r2 = await User.updateMany(
        { 'settings.confidenceThreshold': { $gt: 85 } },
        { $set: { 'settings.confidenceThreshold': 60 } }
      );
...
bot.onText(/\/performance/, async (msg) => {
  await handle(msg, async () => {
    ...
    if (!metrics) {
      await bot.sendMessage(msg.chat.id, '🐆 الفهد — لا توجد صفقات مغلقة خلال 30 يوم بعد');
      return;
    }
    ...
    perfMsg += `   Risk/Trade: ${user?.settings?.dailyRiskPercent || 1}%\n`;
    perfMsg += `   الأزواج: USDT + BTC + ETH\n\n`;
...
bot.on('callback_query', async (query) => {
  ...
  try {
    ...
    if (data.startsWith('close_')) {
      ...
      checkAutoStrictening(userId, bot).catch(e => logger.debug('AutoStrict:', e.message));
      const t = result.trade;
...
bot.onText(/\/forecast(?:\s+(\S+))?/, async (msg, match) => {
  await handle(msg, async () => {
    ...
    const [coins, bt, mtf, liveP] = await Promise.allSettled([
      getTopCoins(), runBacktest(symbol + '/USDT', 'long', 70),
      getMTFAnalysis(symbol + '/USDT', 'daily'), getVerifiedPrice(symbol + '/USDT')
    ]);
    const coinData = (coins.value||[]).find(c => c.symbol === symbol) || { symbol };
    const livePD = liveP.status === 'fulfilled' ? liveP.value : null;
    const enrichedData = { ...coinData, price: (livePD?.price > 0 ? livePD.price : coinData.price) || 0, zScore: mtf.value?.zScore || 0 };
    if (!enrichedData.price) { await bot.sendMessage(msg.chat.id, '🐆 لا يوجد سعر لـ ' + symbol + ' حالياً — حاول مجدداً'); return; }
