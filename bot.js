...
bot.onText(/\/scan|مسح الاسواق/, async (msg) => {
  await handle(msg, async () => {
    ...
    // حفظ الفرص الإضافية مؤقتاً لاستخدامها عند الطلب
    if (extraOpps > 0) {
      pendingScanOpps.set(msg.chat.id, opportunities.slice(5));
      setTimeout(() => pendingScanOpps.delete(msg.chat.id), 10 * 60 * 1000); // تنظيف بعد 10 دقائق
    }
...
bot.on('callback_query', async (query) => {
  ...
  try {
    ...
    if (data === 'show_more_opps') {
      const extras = pendingScanOpps.get(chatId) || [];
      if (!extras.length) {
