...
const { scanMarket, getTopCoins, getFullOnChainData, getPerformanceBenchmarks } = require('./market');
...
function initScheduler(bot) {
  ...
  // ملخص صباحي 10:00 UTC (بعد اغلاق شمعة 09:00 UTC)
  cron.schedule('0 10 * * 1-5', async () => {
    logger.info('الفهد: ملخص صباحي 10:00 UTC');
    try { await sendMorningBriefing(bot); }
    catch (e) { logger.error('خطأ الملخص الصباحي:', e.message); }
  });

  // تداول يومي 10:15 UTC الاثنين-الجمعة
  cron.schedule('15 10 * * 1-5', async () => {
    logger.info('الفهد: تداول يومي 10:15 UTC');
    try { await runDailyAutoTrade(); }
    catch (e) { logger.error('خطأ التداول اليومي:', e.message); }
  });
  ...
  logger.info('🐆 الفهد v2: الجدول الزمني جاهز');
}
