...
async function getHistoricalDaily(symbol, days = 1095) {
  ...
  // /ohlc بـ 365 يوم = شمعات يومية كافية للـ Backtest
  for (const d of [365]) {
    try {
  ...
  // CoinGecko معطّل مؤقتاً
  return [];
}

// ==================== PRICE HISTORY — بناء قاعدة بيانات تاريخية ذاتياً ====================
// يحفظ أسعار يومية من CMC Free تلقائياً — يُبنى تدريجياً
    ...
    try {
      ...
      await PriceHistory.findOneAndUpdate(
        { symbol: coin.symbol, date: today },
        {
          symbol: coin.symbol,
          date: today,
          open: price,   // نستخدم السعر الحالي كـ close
          high: price * (1 + Math.abs(parseFloat(coin.change24h) || 0) / 100),
          low: price * (1 - Math.abs(parseFloat(coin.change24h) || 0) / 100),
          close: price,
...
// جلب البيانات التاريخية من قاعدة البيانات الخاصة
async function getPriceHistoryFromDB(symbol, days = 365) {
  const base = symbol.replace('/USDT', '').toUpperCase();
  try {
    const records = await PriceHistory.find({ symbol: base })
      .sort({ date: 1 })
      .limit(days)
      .lean();
    if (records.length < 5) return [];
    const klines = records.map(r => ({
      time: new Date(r.date).getTime(),
      open: r.open || r.close,
      high: r.high || r.close,
      low: r.low || r.close,
      close: r.close,
      volume: r.volume24h || 0
    }));
    logger.info(`🐆 PriceHistory DB: ${base} ${klines.length} يوم محلي`);
    return klines;
  } catch (e) {
    logger.debug(`🐆 PriceHistory DB فشل ${base}: ${e.message}`);
    return [];
  }
}

// إحصائيات قاعدة البيانات التاريخية
  ...
  try {
    ...
    return {
      ...
      coverage: dates.length > 0
        ? `${dates.length} يوم (${oldestDate} → ${latestDate})`
        : 'لا توجد بيانات بعد'
    };
  } catch (e) { return null; }
}

// ==================== HISTORICAL DATA (DB أولاً + CoinGecko fallback) ====================
async function getHistoricalDaily(symbol, days = 365) {
  const cacheKey = `hist_${symbol}_${days}d`;
  const cached = await getCached(cacheKey);
  if (cached) return cached;

  // محاولة 1: قاعدة البيانات المحلية (تنمو يومياً)
  const dbData = await getPriceHistoryFromDB(symbol, days);
  if (dbData.length >= 10) {
    await setCache(cacheKey, dbData, 6 * 3600);
    return dbData;
  }

  // محاولة 2: CoinGecko /ohlc
  const base = symbol.replace('/USDT', '').toUpperCase();
  const geckoId = GECKO_IDS[base] || base.toLowerCase();
  try {
    const resp = await geckoRequest(
      `https://api.coingecko.com/api/v3/coins/${geckoId}/ohlc`,
      { vs_currency: 'usd', days: 365 }, 20000
    );
    const data = resp.data || [];
    if (data.length >= 5) {
      const klines = data.map(k => ({
      ...
      await setCache(cacheKey, klines, 12 * 3600);
      logger.info(`🐆 CoinGecko ohlc OK: ${symbol} ${klines.length} شمعة`);
      return klines;
    }
  } catch (e) {
    logger.debug(`🐆 CoinGecko ohlc فشل ${symbol}: ${e.response?.status}`);
  }
...
async function getBinanceKlines(symbol, interval = '1d', limit = 365) {
  return getHistoricalDaily(symbol, limit);
}
...
function buildMTFFromCMC(coin) {
  ...
  const s1h = c1h > 0.5 ? 1 : c1h < -0.5 ? -1 : 0;
  const s4h = c24h > 2 ? 1 : c24h < -2 ? -1 : 0;
  const s1d = c7d > 5 ? 1 : c7d < -5 ? -1 : 0;
  const s1w = c30d > 10 ? 1 : c30d < -10 ? -1 : 0;
  ...
  const tfDetails = [
    { tf: '1H', trend: s1h > 0 ? 'bullish' : s1h < 0 ? 'bearish' : 'neutral', rsi: Math.min(80, Math.max(20, 50 + c1h * 3)), bbPosition: null },
    { tf: '4H', trend: s4h > 0 ? 'bullish' : s4h < 0 ? 'bearish' : 'neutral', rsi: Math.min(80, Math.max(20, 50 + c24h * 1.5)), bbPosition: null },
    { tf: '1D', trend: s1d > 0 ? 'bullish' : s1d < 0 ? 'bearish' : 'neutral', rsi: Math.min(80, Math.max(20, 50 + c7d * 0.8)), bbPosition: null },
    { tf: '1W', trend: s1w > 0 ? 'bullish' : s1w < 0 ? 'bearish' : 'neutral', rsi: Math.min(80, Math.max(20, 50 + c30d * 0.4)), bbPosition: null }
  ];
  return {
    timeframes: tfDetails.map(t => ({ tf: t.tf, analysis: { trend: t.trend, rsi: t.rsi, signal: t.trend === 'bullish' ? 1 : t.trend === 'bearish' ? -1 : 0, zScore: 0, zInterpret: { signal: 'N/A', emoji: '' } } })),
  ...
  try {
    if (type === 'daily') {
      const [h1, h4, d1] = await Promise.allSettled([
        getKlines(symbol, '1h'), getKlines(symbol, '4h'), getKlines(symbol, '1d')
      ]);
      const klines = [h1, h4, d1].map(r => r.status === 'fulfilled' ? r.value : []);
      if (klines.some(k => k.length >= 10)) {
        timeframeData = [
          { tf: '1H', klines: klines[0] },
          { tf: '4H', klines: klines[1] },
          { tf: '1D', klines: klines[2] }
        ];
        geckoOk = true;
      }
    } else {
      const d1 = await getKlines(symbol, '1d').catch(() => []);
      if (d1.length >= 10) {
        timeframeData = [
          { tf: '1D', klines: d1 },
    ...
    logger.debug('MTF CoinGecko: ' + e.message);
  }

  // CMC fallback إذا فشل CoinGecko
  if (!geckoOk) {
    const base = symbol.replace('/USDT', '').toUpperCase();
    const coins = await getCached('top100_coins_v2') || [];
    const coin = coinData || coins.find(c => c.symbol === base);
    if (coin) {
      const result = buildMTFFromCMC(coin);
      await setCache(cacheKey, result, 1800);
      logger.info('MTF CMC fallback: ' + symbol);
      return result;
    }
    // إذا لم نجد البيانات — نرجع محايد
    const neutral = buildMTFFromCMC({ change1h: 0, change24h: 0, change7d: 0, change30d: 0 });
    return neutral;
  }

  // معالجة بيانات CoinGecko
...
async function runBacktest(symbol, signalType, confidence) {
  ...
  try {
    ...
    // إذا كانت البيانات أقل من 15 شمعة، استخدم getKlines مباشرة
    if (klines.length < 15) {
      klines = await getKlines(symbol, '1d');
      logger.info(`🐆 Backtest getKlines fallback: ${symbol} ${klines.length} شمعة`);
    }
    ...
    const minWindow = Math.min(5, Math.floor(closes.length / 4));
    const step = Math.max(1, Math.floor(closes.length / 100));
    logger.info(`🐆 Backtest loop: ${symbol} closes=${closes.length} step=${step} minW=${minWindow}`);
    for (let i = minWindow; i < closes.length - 3; i += step) {
      const window = closes.slice(Math.max(0, i - minWindow), i);
      if (window.length < 3) continue;
      const rsi = calculateRSI(window, Math.min(7, window.length - 1));
      const ema20 = calculateEMA(window, Math.min(minWindow, window.length));
      const longSignal = signalType === 'long' && (closes[i] > ema20 || rsi < 35);
      const shortSignal = signalType === 'short' && (closes[i] < ema20 || rsi > 65);
...
function buildMinimalBacktest(symbol, klines, signalType) {
  ...
  const closes = klines.map(k => k.close);
  let wins = 0, losses = 0;
  for (let i = 5; i < closes.length - 3; i++) {
    const w = closes.slice(0, i);
    const rsi = calculateRSI(w);
    const ema = calculateEMA(w, Math.min(10, w.length));
...
module.exports = {
  getTopCoins, getVerifiedPrice, getKlines, getBinanceKlines, getHistoricalCached, getHistoricalDaily,
