...
function formatScan(opportunities, totalScanned, summary, onChain) {
  ...
  msg += ` | ${opportunities.length} فرصة فوق 60%\n\n`;
  ...
  const extraCount = opportunities.length - top5.length;
  ...
  top5.forEach((o, i) => {
    ...
    const rr = parseFloat(o.riskReward || 0);
    const rrText = rr >= 0.1 ? rr.toFixed(1) + ':1' : 'يُحسب';
    msg += `   R/R: ${rrText} | MTF: ${((o.mtfAlignment || 0) * 100).toFixed(0)}%\n\n`;
  });

  // إشعار الفرص الإضافية
  if (extraCount > 0) {
    msg += `\n📋 هذه أفضل 5 فرص وفق معايير الاستراتيجية (ثقة، MTF، R/R).\n`;
    msg += `يوجد ${extraCount} فرصة إضافية — هل تريد مشاهدتها؟\n`;
    msg += `${DIVIDER}\n`;
  }
...
function formatAnalysis(analysis, symbol) {
  ...
  if (hasPrice) {
    ...
    const rrVal = parseFloat(analysis.riskReward || 0);
  msg += `   R/R: ${rrVal >= 0.1 ? rrVal.toFixed(1) + ':1' : 'يُحسب'}\n`;
  }
...
function formatTrade(trade, isNew = true) {
  ...
  if (trade.entryDeadline) {
    const deadlineDate = new Date(trade.entryDeadline);
    const remaining = Math.max(0, Math.round((deadlineDate - Date.now()) / 3600000));
    const deadlineTime = deadlineDate.toLocaleTimeString('ar', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Riyadh' });
    const deadlineDay = deadlineDate.toLocaleDateString('ar', { weekday: 'short', timeZone: 'Asia/Riyadh' });
    if (remaining > 0) {
      msg += `   ⏰ ادخل قبل: ${deadlineTime} ${deadlineDay} (${remaining} ساعة متبقية)\n`;
    } else {
      msg += `   ⏰ انتهت مهلة الدخول\n`;
    }
  }
  if (trade.expiresAt) {
    const expDate = new Date(trade.expiresAt).toLocaleDateString('ar', { timeZone: 'Asia/Riyadh' });
    msg += `   📅 انتهاء الصفقة إن دخلت: ${expDate}\n`;
  }
  if (trade.backtestSummary) {
    if (trade.backtestSummary.indexOf('0 مرة') === -1) {
      msg += '\n📉 Backtest: ' + safe(trade.backtestSummary, 100) + '\n';
    } else {
      // رسالة واحدة فقط — لا تكرار
      msg += '\n📊 إشارات تاريخية: بيانات تتراكم — التحليل مبني على MTF + On-Chain\n';
    }
  }
  if (trade.analysisSnapshot) {
    var snap = stripDividers(safe(trade.analysisSnapshot, 220));
    if (snap) msg += '\n💡 ' + snap + '\n';
  }
  msg += `${DIVIDER}`;
  return msg;
...
function formatMorningBriefing(briefing, scan, benchmarks) {
  ...
  if (benchmarks?.eth) msg += `Ξ ETH: ${fmtPrice(benchmarks.eth.price, 0)} (${fmtPct(benchmarks.eth.change24h)})\n`;
  if (fg) msg += `😰 الخوف والطمع: ${fg.value}/100 — ${fg.classificationAr}\n`;
  msg += `🔍 فرص Spot اليوم (60%+): ${scan?.opportunities?.length || 0} عملة\n\n`;
  msg += `📋 ملخص ${FAHD}:\n${safe(briefing, 600)}`;
  return msg;
}
