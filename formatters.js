...
function formatScan(opportunities, totalScanned, summary, onChain) {
  ...
  if (fg || dom) {
    ...
    var domVal = typeof dom === 'object' ? (dom.btcDominance || dom.value || 'N/A') : dom;
    var domSig = typeof dom === 'object' && dom.signal ? ' — ' + safe(dom.signal) : '';
    msg += '   هيمنة BTC: ' + domVal + '%' + domSig + '\n';
  }
    msg += '\n';
  }
...
function formatTrade(trade, isNew = true) {
  ...
  // Backtest — رسالة واضحة بدلاً من 0%
  ...
  if (trade.analysisSnapshot) {
    var snap = stripDividers(safe(trade.analysisSnapshot, 250));
    if (snap) msg += '\n💡 ' + snap + '\n';
  }
  msg += `${DIVIDER}`;
  return msg;
