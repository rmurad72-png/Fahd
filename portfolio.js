...
async function closeTrade(userId, tradeId, reason = 'manual') {
  ...
  logger.info(`🐆 الفهد v2: إغلاق ${trade.symbol} PnL=$${pnl.toFixed(2)} (${pnlPercent.toFixed(2)}%)`);
...
async function updateTrailingStop(trade, currentPrice) {
  ...
  if (newStop > (trade.currentStopLoss || trade.stopLoss)) {
    ...
    logger.info(`🐆 Trailing Stop: ${trade.symbol} → $${newStop.toFixed(6)}`);
    return newStop;
  }
  return null;
}
    ...
    try {
      ...
      if (newStop && bot) {
        await bot.sendMessage(trade.userId,
          `📈 الفهد — تحديث Trailing Stop\n${trade.symbol}\nوقف جديد: $${newStop.toFixed(6)} (50% من المكسب محمي)`
        );
      }
...
async function checkCapitalProtection(userId, user) {
  ...
  if (drawdown >= 20) {
    ...
    logger.warn(`🛡️ حماية رأس المال: ${userId} — انخفاض ${drawdown.toFixed(1)}%`);
    return { blocked: true, reason: `انخفاض ${drawdown.toFixed(1)}% من الذروة — تم إيقاف التداول الآلي`, drawdown };
  }
