...
async function openTrade(userId, tradeData) {
  ...
  // مهلة ديناميكية حسب تقلبية العملة
  const change24h = Math.abs(parseFloat(tradeData.change24h || 0));
  let entryHours;
  if (type === 'monthly') {
    entryHours = 36;
  } else if (change24h >= 8) {
    entryHours = 2;  // تقلب عالٍ جداً → دخول سريع
  } else if (change24h >= 4) {
    entryHours = 4;  // تقلب عالٍ
  } else if (change24h >= 2) {
    entryHours = 8;  // تقلب معتدل
  } else {
    entryHours = 16; // تقلب منخفض
  }
  const entryDeadline = new Date(Date.now() + entryHours * 3600 * 1000);
...
async function updateTrailingStop(trade, currentPrice) {
  ...
  // تفعيل بعد ربح >= 1x المسافة الأولية (min 3%)
  const initialDistance = Math.abs(trade.entryPrice - trade.stopLoss) / trade.entryPrice;
  const activationPct = Math.max(initialDistance * 100, 3);
  if (pnlPercent < activationPct) return null;

  const profit = currentPrice - trade.entryPrice;
  const initialStopDist = trade.entryPrice - trade.stopLoss;

  // Breakeven Stop: عند 1.5x المسافة الأولية → وقف إلى نقطة الدخول + 0.1%
  const breakevenActivation = initialDistance * 1.5 * 100;
  let newStop;

  if (pnlPercent >= breakevenActivation) {
    // الخيار 1: 50% من الربح المحقق
    const stop50pct = trade.entryPrice + (profit * 0.5);
    // الخيار 2: ATR-based (نستخدم 1.5x المسافة الأولية كتقريب للـ ATR)
    const stopATR = currentPrice - (initialStopDist * 1.5);
    // نختار الأوسع (الأعلى قيمة = الأكثر حماية)
    newStop = Math.max(stop50pct, stopATR, trade.entryPrice * 1.001); // على الأقل breakeven + 0.1%
  } else {
    // قبل الـ breakeven: 50% من الربح فقط
    newStop = trade.entryPrice + (profit * 0.5);
  }

  if (newStop > (trade.currentStopLoss || trade.stopLoss)) {
    const isBreakeven = newStop >= trade.entryPrice * 1.001;
    await Trade.updateOne(
      { tradeId: trade.tradeId },
      { $set: { currentStopLoss: newStop, highestPrice: Math.max(currentPrice, trade.highestPrice || currentPrice) } }
    );
    logger.info(`🐆 Trailing Stop: ${trade.symbol} → ${(newStop || 0).toFixed(6)} (${isBreakeven ? 'Breakeven+' : '50% ربح'})`);
    return { newStop, isBreakeven };
  }
  return null;
}
...
async function monitorOpenTrades(bot = null) {
  ...
  for (const trade of trades) {
    try {
      ...
      // تحديث Trailing Stop
      const trailingResult = await updateTrailingStop(trade, price);
      if (trailingResult && bot) {
        const { newStop, isBreakeven } = trailingResult;
        const stopMsg = isBreakeven
          ? `✅ Breakeven محمي — وقف فوق نقطة الدخول`
          : `50% من المكسب محمي`;
        await bot.sendMessage(trade.userId,
          `📈 الفهد — تحديث Trailing Stop\n${trade.symbol}\nوقف جديد: ${(newStop || 0).toFixed(6)}\n${stopMsg}`
        );
      }
...
module.exports = {
  ...
  checkAutoStrictening, getPerformanceMetrics
};
...
async function checkAutoStrictening(userId, bot) {
  try {
    ...
    const now = new Date();
    const window30 = new Date(now - 30 * 24 * 3600 * 1000);

    // جلب صفقات الـ 30 يوم الأخيرة
    const recentTrades = await Trade.find({
      userId, status: 'closed',
      closedAt: { $gte: window30 }
    }).sort({ closedAt: -1 });

    const wins = recentTrades.filter(t => (t.pnl || 0) > 0).length;
    const losses = recentTrades.filter(t => (t.pnl || 0) <= 0).length;
    const totalPnL30 = recentTrades.reduce((s, t) => s + (t.pnl || 0), 0);
    const pnlPct30 = user.portfolio.initialBalance > 0
      ? (totalPnL30 / user.portfolio.initialBalance * 100) : 0;

    // حساب Drawdown الحالي
    const snap = await getPortfolioSnapshot(userId);
    const drawdown = snap ? parseFloat(snap.drawdown || 0) : 0;

    const consecutive = user.stats.consecutiveLosses || 0;
    const isStrict = user.settings.strictMode || false;
    ...
    // ============ شروط الإلغاء ============
    const strictWins = user.stats.strictWinStreak || 0;
    const recoveredHalf = isStrict && totalPnL30 > 0 && pnlPct30 >= 5;
    const shouldDeactivate = isStrict && (strictWins >= 5 || recoveredHalf);

    if (shouldActivate) {
      // تحديد السبب
      const reasons = [];
      if (trigger3Losses) reasons.push(`${consecutive} خسائر متتالية`);
      if (trigger10PctLoss) reasons.push(`خسارة ${Math.abs(pnlPct30).toFixed(1)}% خلال 30 يوم`);
      if (trigger12Drawdown) reasons.push(`Max Drawdown ${drawdown.toFixed(1)}%`);

      // تطبيق التشديد فوراً
      const newRisk = parseFloat((user.settings.dailyRiskPercent * 0.5).toFixed(2));
      const newThreshold = Math.min((user.settings.confidenceThreshold || 60) + 10, 85);
      await User.updateOne({ telegramId: userId }, {
        $set: {
          'settings.strictMode': true,
          'settings.strictModeActivatedAt': now,
          'settings.riskMultiplier': 0.5,
          'settings.dailyRiskPercent': newRisk,
          'settings.confidenceThreshold': newThreshold,
          'stats.strictWinStreak': 0
        }
      });

      // إرسال التقرير
      if (bot) {
        const report = `🚨 الفهد — تقرير التشديد التلقائي\n━━━━━━━━━━━━━━━━━━━━\n\n⚠️ تم تفعيل وضع التشديد فوراً\n\n📋 الأسباب:\n${reasons.map(r => `• ${r}`).join('\n')}\n\n📉 الإجراءات المُطبَّقة:\n• Risk per trade: ${user.settings.dailyRiskPercent}% → ${newRisk}%\n• حد الثقة: ${user.settings.confidenceThreshold}% → ${newThreshold}%\n• إضافة المراكز: موقوفة\n• التشديد يُلغى تلقائياً بعد 5 صفقات رابحة أو استعادة 50% من الخسارة\n\n📊 إحصائيات الفترة (30 يوم):\n• صفقات رابحة: ${wins}\n• صفقات خاسرة: ${losses}\n• PnL: ${totalPnL30 >= 0 ? '+' : ''}$${totalPnL30.toFixed(2)} (${pnlPct30.toFixed(1)}%)\n• Drawdown الحالي: ${drawdown.toFixed(1)}%\n\n⏰ مراجعة إجبارية خلال 72 ساعة`;
        await bot.sendMessage(userId, report);
      }
      logger.warn(`🚨 Auto-Strict تفعيل: ${userId} — ${reasons.join(', ')}`);
      ...
      await User.updateOne({ telegramId: userId }, {
        $set: {
          ...
          'settings.riskMultiplier': 1.0,
          'settings.dailyRiskPercent': Math.min((user.settings.dailyRiskPercent * 2), 3),
      ...
      if (bot) {
        const report = `✅ الفهد — إلغاء التشديد التلقائي\n━━━━━━━━━━━━━━━━━━━━\n\n🟢 تم إلغاء وضع التشديد — عودة للإعدادات العادية\n\n📋 السبب:\n• ${strictWins >= 5 ? `${strictWins} صفقات رابحة متتالية` : 'استعادة 50% من الخسارة'}\n\n📊 إحصائيات الفترة (30 يوم):\n• صفقات رابحة: ${wins}\n• PnL: ${totalPnL30 >= 0 ? '+' : ''}$${totalPnL30.toFixed(2)} (${pnlPct30.toFixed(1)}%)\n• Drawdown: ${drawdown.toFixed(1)}%\n\n⚙️ الإعدادات المستعادة:\n• Risk per trade: 3%\n• حد الثقة: 60%`;
        await bot.sendMessage(userId, report);
      }
      logger.info(`✅ Auto-Strict إلغاء: ${userId}`);
    }

    // تحديث عداد الانتصارات في وضع التشديد
    if (isStrict) {
      const lastTrade = recentTrades[0];
      if (lastTrade && (lastTrade.pnl || 0) > 0) {
        await User.updateOne({ telegramId: userId }, { $inc: { 'stats.strictWinStreak': 1 } });
      } else if (lastTrade && (lastTrade.pnl || 0) <= 0) {
        await User.updateOne({ telegramId: userId }, { $set: { 'stats.strictWinStreak': 0 } });
      }
    }

  } catch (e) {
    logger.error('checkAutoStrictening:', e.message);
  }
}
...
async function getPerformanceMetrics(userId) {
  try {
    const window30 = new Date(Date.now() - 30 * 24 * 3600 * 1000);
    const trades = await Trade.find({ userId, status: 'closed', closedAt: { $gte: window30 } });
